import { Scheduler, Timer } from "./Scheduler.js";
...
class Sphere {
  constructor(options) {
    ...
    // Uniforms
    this.alpha = { value: 1 }; // TODO: Multiply blending w/ alpha?
    ...
    this.lig.addAttribute(
      "displayParam",
      new THREE.InstancedBufferAttribute(this.lDisplayParams, 1)
    );
  ...
  // TODO: Replace vec3 (?), might be able to move calc to vs
  updateCircleOffsets() {
    ...
    this.cOffsets.fill(0, this.nCircles * 3); // TODO: Might be unnecessary due to scale clearing
  }
  ...
  // Counter-clockwise along xz surface
  updateLineRotations() {
  ...
  startAnimation() {
    ...
    var circleCB = function (scheduler, now) {
      scheduler.addTask({ condition: now + 14000, runCallback: circleCB2 });
